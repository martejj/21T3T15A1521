...
main_scan_start:
    ...
    li $v0, 5
    syscall                                 # scanf("%d", &input); where input = $v0

    li $t1, 4                               # $t1 = 4
    mul $t1, $t1, $t0                       # $t1 = 4*i
    la $t2, numbers
    add $t1, $t1, $t2                       # $t1 = numbers + 4*i => int *address = numbers + 4*i;
    
    sw $v0, ($t1)                           # *address = input;
...
main_print_start:
    ...
    lw $a0, ($t1)                           # int output = *address;
    li $v0, 1
    syscall                                 # printf("%d", output);
